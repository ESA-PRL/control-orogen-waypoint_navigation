...
bool TrajectoryTest::configureHook()
{
    ...
    if (xpos.size() != ypos.size())
    {
        std::cerr << "\nError in TrajectoryTest::configureHook()\n"
                  << "xpos size (" << xpos.size() << ") does not match ypos (" << ypos.size()
                  << ") size!\n"
    ...
    {
        std::cerr << "\nError in TrajectoryTest::configureHook()\n"
                  << "Index: " << *result << " exceeds the number of available waypoints ("
                  << xpos.size() << ").\n"
    ...
    // Now iteratively set the trajectory so that each previous waypoints has a heading towards the
    // next waypoint.
    int it;
    for (it = N - 1; it >= 0; it--)
    {
...
{
    if (!TrajectoryTestBase::startHook()) return false;
...
void TrajectoryTest::errorHook() { TrajectoryTestBase::errorHook(); }
void TrajectoryTest::stopHook() { TrajectoryTestBase::stopHook(); }
void TrajectoryTest::cleanupHook() { TrajectoryTestBase::cleanupHook(); }
